...
const Topic = ({ topic }) => {
  ...
  return (
    <div className="card card-compact h-full bg-white border-2 border-cyan-500 shadow-xl p-3 m-3">
      <figure>
        <img src={logo} alt="" className="bg-yellow-300" />
      </figure>
      <div className="card-body flex flex-col p-3 justify-around">
        <h2 className="card-title text-lg text-purple-700 font-bold">{name}</h2>
        <p className="pb-3 text-indigo-600 font-semibold">Total: {total}</p>
        <Link
          to={`/topic/${id}`}
